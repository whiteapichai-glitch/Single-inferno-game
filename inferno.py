...
if st.session_state.step == "SETUP":
    ...
    st.write("ตั้งชื่อและอัปโหลดรูปสมาชิกทั้ง 12 คน (ระบบจะเริ่มเกมด้วยชาย 4 หญิง 4 คนแรกจ้ะ)")
    ...
    with col1:
        ...
        for i in range(6):
            ...
            file = st.file_uploader(f"อัปโหลดรูป {name}", type=['jpg','png','jpeg'], key=f"m_u{i}")
            img = file if file else DEF_M_IMG
            m_inputs.append({"name": name, "img": img, "gender": "M"})
            st.divider()
    ...
    with col2:
        ...
        for i in range(6):
            ...
            file = st.file_uploader(f"อัปโหลดรูป {name}", type=['jpg','png','jpeg'], key=f"f_u{i}")
            img = file if file else DEF_F_IMG
            f_inputs.append({"name": name, "img": img, "gender": "F"})
            st.divider()
        ...
        with t_entry:
            ...
            if waiting:
                ...
                if privilege:
                    ...
                    partner_name = st.selectbox("เลือกคู่เดทที่จะพาไปด้วย:", avail_partners)
